...
test('fails closed when a verifier completion omits scoped source inspection', async () => {
  const targetRoot = await mkdtemp(join(tmpdir(), 'security-reviewer-verification-stage-'));
  ...
  provider.enqueueObject({
    object: {
      ...
      unsafeConditionEvidence: { factId: 'fact-source-01', evidenceIndex: 0 },
      controlAssessment: {
  ...
  const result = await runVerificationStage({
    ...
    request: AuditVerificationRequestSchema.parse({
      ...
      hypothesis: {
        vectorId: 'vector-unknown-01',
        statement: 'Request-controlled value reaches a query',
        evidence: [
          {
            path: 'reviewed.unknown',
            startLine: 1,
            snippet: 'query = request.input',
            kind: 'source',
            role: 'operation',
          },
          {
        ...
        facts: [
          {
            ...
            role: 'operation',
            statement: 'The approved source contains query construction.',
            evidence: [
              {
                path: 'reviewed.unknown',
                startLine: 1,
