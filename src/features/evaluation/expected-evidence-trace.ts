import type { AttackPlan, SourceEvidence } from '../attack-planning/plan.schema.js';
import type { VectorCoverage } from '../audit-execution/audit.schema.js';
...
/** Records a validated discovery seed only through its selected evidence-map facts. */
export function traceDiscoveryEvidence(
  ...
  return update(
    ...
    answerKey,
    (expected) =>
      evidenceMapFactIds.some((factId) => {
        const fact = factsById.get(factId);
        return fact !== undefined && evidenceMatches(expected, fact.evidence, false);
      }),
    'discoverySeeded',
