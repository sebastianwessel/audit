...
export async function importCorpusPack(input: {
  ...
  try {
    ...
    for (const sourcePath of selected) {
      ...
      content.push({ relativePath, digest: sha256(await readFile(sourcePath, 'utf8')) });
    }
...
function importError(message: string): SecurityReviewerError {
  return new SecurityReviewerError('invalid-input', `Invalid offline corpus import: ${message}`);
}
