  ...
  return EvaluationRunComparisonSchema.parse({
    ...
    metrics: comparable
      ? {
          ...
          durationMsP95: metric(
            baselineRun.reliability.durationMsP95,
            candidateRun.reliability.durationMsP95,
          ),
...
export function renderEvaluationRunComparison(comparison: EvaluationRunComparison): string {
  const lines = [
    '# Security Reviewer evaluation comparison',
    '',
  ...
  lines.push(
    ...
    ...Object.entries(comparison.metrics).map(([name, value]) =>
      metricRow(name, value.baseline, value.candidate, value.delta),
    ),
    '',
    '## Per-stage deltas',
    '',
