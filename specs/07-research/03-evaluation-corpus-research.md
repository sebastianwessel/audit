...
For a configured provider/model, run the identical reviewed target view with an explicitly recorded repeat count outside default CI. Report only the medians, ranges, and agreement values that the observed trial count supports, alongside plan category recall, finding recall/precision, pair persistence rate, cost, latency, tool-call counts, and invalid outputs. A single run is a smoke result, not a reliability claim.
