...
| REQ-041 | Every investigator hypothesis is independently accepted, rejected, or marked incomplete by a bounded verifier stage before it can persist. The verifier may inspect only the same vector scope, cannot create a finding, and must seek source-local controls that could negate the claim before acceptance. It has separately recorded request/token/cost/tool observations. |
