...
| EVAL-090 | Measure semantic generated-plan coverage through one explicit AI-assisted development adjudication. | `evaluation/plan-semantic-adjudication` | A strict evaluator-only artifact binds the exact generated plan and answer-key scenario set; deterministic code validates closure and derives metrics without using wording, category, source, parser, or benchmark heuristics. It is diagnostic only. |
...
`PlanSemanticRunEvaluation` v2 aggregates only completed per-trial semantic artifacts for one generated-plan run. It binds run id, pack id/version, split, and prompt protocol; exposes eligible, adjudicated, and missing-adjudication trial counts; and derives global scenario recall, relevant-vector precision, unrelated vectors, and duplicate relevant vectors from adjudicated trials only. v2 carries the explicit `single-ai-assisted-development-review` qualification: it is diagnostic evidence only. An absent artifact is unavailable evidence, never a zero score. It cannot support external reliability, independent validation, provider selection, product admission, or a workflow gate.
