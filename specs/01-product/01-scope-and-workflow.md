...
| Reviewer | Manual plan edit | Decide scope and plan content outside the product workflow. |
...
plan validates paths/config, inventories files, exposes scoped read-only evidence, runs a threat-aware planner, validates the response, and writes an executable JSON/Markdown plan.
