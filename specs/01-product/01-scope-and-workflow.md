...
An authorized developer or CI job supplies the source repository to review and optional per-use-case Markdown context. Security Reviewer creates a scoped review plan. A human may review, edit, extend, enable, or disable it in their own process. The supplied strict plan executes directly when its target and context fingerprints match. The audit produces evidence-backed confirmed claims. Classification, urgency, confidence language, impact summaries, and remediation are explicitly outside this confirmation protocol.
...
In scope are repository files, explicitly allowlisted Markdown/frontmatter context, deterministic inventory/search, model reasoning over scoped evidence, attack-vector planning, evidence-backed findings, data-protection review, and report-only remediation advice.
...
| CI | Exit code and JSON | Fail when one or more confirmed claims are present; incomplete, failed, or cancelled coverage remains a distinct operational outcome. |
| Provider | Purista adapter | Receive scoped, configured model input. |

## Inputs, admission, and context

The target, context, and output roots are canonical directories and must be pairwise disjoint in both ancestor directions before the product creates a directory, inventories a file, loads an artifact, or constructs a provider. The output root is never a target or context input. `03-architecture/09-immutable-evidence-and-terminal-truth.md` owns this boundary.
...
Context is untrusted input. The system validates its path, frontmatter, size, encoding, and allowed document type; labels it as context rather than source; and treats instructions inside the Markdown body as data, not commands. A context claim that conflicts with source evidence is reported as an assumption or limitation and does not silently suppress a finding.
...
report formats an existing valid JSON report without model calls.
...
| 1 | Every enabled obligation closed; one or more confirmed claims. |
