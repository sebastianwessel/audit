...
1. **Hypothesis discovery** decides whether scoped mapped/posture evidence warrants a review hypothesis for each approved obligation. It produces a small, non-reportable seed or an explicit closure. A seed is not a finding and contains no classification, urgency, impact, remediation, source snippet, source-evidence role, source-flow assertion, or model-authored posture identifier. The posture owner derives the exact posture references from approved obligations.
2. **Canonical grounding** receives only discovery seeds for the same vector together with the validated map, posture, scoped path manifest, advisory context, and read-only tools. It either returns `null` for a seed or one complete candidate whose plan/map references satisfy the ordinary canonical admission boundary; canonical materialization carries the seed-owned posture projection. A non-null candidate contains only a precise statement, limitations, obligation/map references, and operation/unsafe-condition selections. It selects those locations solely as `{ factId, evidenceIndex }` from its own selected neutral map facts; deterministic code projects the source evidence. It may not create a seed, vector, classification, urgency, impact, remediation, scope, source path, line, snippet, evidence role, or posture identifier. It never emits or depends on a synthetic trace pointer.
3. **Verification** remains the source-scoped falsifier for each canonical candidate. It may accept, reject, or mark incomplete; it cannot create a candidate or widen scope.
...
- A discovery seed has one stable seed id, its exact vector id, one or more approved plan-obligation references, non-empty selected map references, a bounded neutral hypothesis statement, and limitations. The posture owner derives the non-empty posture references and included map facts for those obligations before grounding. A `candidate-raised` closure requires at least one valid seed for the same obligation; `no-source-backed-candidate` requires none; `incomplete` remains visible.
...
- A grounding result contains exactly one entry for every requested seed. Its entry is either `null` or a complete model-facing candidate. A non-null candidate preserves the seed's exact vector, plan-obligation, and map references; canonical materialization carries the seed-owned posture projection. It names one operation and one unsafe-condition selection, each a selected map `factId` plus an in-range `evidenceIndex`; deterministic code projects the selected source evidence and sets its role. This binds an initial claim to its neutral evidence basis without interpreting the fact roles or source semantics. The verifier independently reads scoped source for its decision, but an accepted result selects its operation, unsafe-condition, and control evidence from that same map artifact; code projects the exact obligations and relevant control identifiers. Deterministic code does not infer a vulnerability, data flow, relationship, control effectiveness, or priority.
...
- Only canonical, redacted grounded hypotheses, validated closures, closed source-free grounding funnels/rejection counts, and content-free discovery/grounding observations may be checkpointed for verifier retry. Discovery seeds, raw discovery output, raw grounding output, prompts, source text, tool arguments/results, provider ids, and credentials are never artifacts. A resumed verifier carries the saved funnels and observations to terminal coverage rather than deriving replacement values from the saved hypotheses. Map/posture recovery remains reusable; interruption before canonical grounding resumes from the latest safe predecessor.
...
- A newly fingerprinted five-repeat reviewed-plan provider evaluation is recorded descriptively after local checks. It must report detection, false-positive, attrition, completion, cost, tokens, and latency; it must not auto-promote a baseline or alter behavior from answer keys.
