...
The feature computes each report finding's deterministic identity with the single-owned finding identity function. That identity is derived from the vector id, precise claim statement, and primary source location, then compared exactly. It does not use snippets, embeddings, language hints, parsers, classification, urgency, source text, or model reasoning to create a fuzzy/root-cause match.
