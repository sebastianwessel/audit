...
| candidate grounding | seed partition, with each seed retaining only its approved scope | Reassemble exactly one result per original seed. | Conflicting non-null results, a missing seed, or a failed child produces no canonical grounding draft. |
| verification/countercheck | one hypothesis | Retry with provider-requested source or applicable-context partitions only to collect source-local evidence; a verifier decision is admitted only when one complete schema-valid decision covers the original hypothesis. | A decision cannot be losslessly merged across children; return `incomplete` with the context-overflow limitation. |
