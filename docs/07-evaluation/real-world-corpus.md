...
Normally the provider and model come from `.env`, so the shortest form is `bun run eval:provider`. Command flags are useful for a one-off comparison. Use `--verification-mode same-route` to measure the default product route even if `.env` configures an evaluation-only independent verifier. Live trials use a 120-second model deadline and a 150-second run deadline. The model deadline is enforced by both the review runtime and the provider connection. By default, a failed agent invocation gets one fresh same-input retry while keeping the original vector scope; a normalized context-window error instead uses deterministic scope recovery. A continuing provider problem remains a failed trial. Override both deadlines together when needed: `--model-timeout-ms 120000 --run-timeout-ms 150000`.
