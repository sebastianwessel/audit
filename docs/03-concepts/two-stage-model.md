...
Many security tools start analysing immediately. Audit starts with a conversation about scope.
...
The auditor investigates enabled vectors independently, using the configured concurrency. For each vector it first maps neutral source facts, relevant controls, and unanswered questions inside the plan’s file scope. Only then can it form a map-bound hypothesis and have a verifier challenge it. Each model stage receives the same complete plan-scoped path manifest and read-only file tools; the tool does not use a hidden language-specific detector to decide where a vulnerability exists.

Before a result becomes a finding, the reviewer verifies that its vector, file, line, and source-minimal evidence reference all match the plan-scoped evidence. The finding also identifies which plan-owned review obligations it answers; the independent verifier must confirm that link as well as identify both a security-relevant operation and why it is unsafe from its own scoped source inspection. The operation must be the source location that actually performs the relevant action—not merely a nearby lookup, declaration, or setup step. A single keyword or API name is not enough. Invalid, contradicted, or out-of-scope suggestions are visible as incomplete evidence; they are not quietly promoted into the report. If inspected evidence shows a business-level check does not apply, it is marked not applicable with a clear closed reason; that is neutral, not a passed check or finding.
...
sequenceDiagram
  ...
  A-->>O: source-minimal findings and coverage state
~~~

## Plan review is external

The plan is an ordinary artifact that can be reviewed, stored, edited, approved, and compared in your organization’s own workflow. Audit deliberately does not store or enforce that approval state. It executes a strict plan only when its target and context fingerprints match the current audit input.
