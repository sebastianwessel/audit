...
/** Evaluation entrypoints handle this before configuration, artifact, or provider work. */
export function isEvaluationHelpRequest(argv: readonly string[]): boolean {
  const argumentsWithoutSeparator = argv[0] === '--' ? argv.slice(1) : argv;
  return argumentsWithoutSeparator.length === 1 && argumentsWithoutSeparator[0] === '--help';
}

function invalidOptionPairs(): AuditRuntimeError {
