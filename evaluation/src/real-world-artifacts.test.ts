...
test('serializes a provider evaluation run and fails closed for an unresolved lock', async () => {
  const root = join(tmpdir(), `audit-provider-lock-${crypto.randomUUID()}`);
  ...
  const runDirectory = join(root, '.provider-evaluations', runId);
...
test('keeps semantic evaluator checkpoints in the private work root', () => {
  expect(
    evaluationPlanSemanticCheckpointPath('evaluation-run-01', 'evaluation-trial-semantic-01'),
  ).toBe(
    '.provider-evaluations/evaluation-run-01/work/plan-semantic-evaluations/evaluation-trial-semantic-01.checkpoint.json',
  );
});

test('atomically retains a published evaluator attempt before unfinished recovery', async () => {
  const root = join(tmpdir(), `audit-evaluator-retain-${crypto.randomUUID()}`);
  const runId = 'provider-retain-test';
  const attemptId = 'provider-retain-attempt-01';
  const terminalPath = join(root, runId);
  await mkdir(terminalPath, { recursive: true });
  await writeFile(join(terminalPath, 'evaluation-run.json'), '{"safe":true}\n', 'utf8');
  ...
  await expect(
    ...
    archivePath: `.provider-evaluations/${runId}/published-attempts/${attemptId}`,
    alreadyArchived: false,
  });
  await expect(Bun.file(join(terminalPath, 'evaluation-run.json')).exists()).resolves.toBe(false);
  const archived = join(
    root,
  ...
  await expect(Bun.file(archived).text()).resolves.toBe('{"safe":true}\n');
  await expect(
    archivePublishedProviderEvaluationArtifacts({
      outputRoot: root,
      runId,
...
test('inspects source-free ownership and releases only an exactly confirmed provider lock', async () => {
  const root = join(tmpdir(), `audit-provider-lock-operator-${crypto.randomUUID()}`);
  const runId = 'provider-lock-operator-test';
  const attemptId = 'provider-lock-attempt-01';
  const checkpointFingerprint = 'a'.repeat(64);
  await acquireProviderEvaluationLock(root, runId, {
    attemptId,
    checkpointFingerprint,
    createdAt: '2026-08-03T12:00:00.000Z',
    commandMode: 'resume-work',
  });
  const retainedArtifact = join(root, '.provider-evaluations', runId, 'retained-artifact.json');
  await writeFile(retainedArtifact, '{"retained":true}\n', 'utf8');

  await expect(inspectProviderEvaluationLock({ outputRoot: root, runId })).resolves.toMatchObject({
    lock: {
      runId,
      attemptId,
      checkpointFingerprint,
  ...
  await expect(
    releaseProviderEvaluationLock({
      outputRoot: root,
      runId,
      attemptId: 'provider-lock-attempt-02',
      checkpointFingerprint,
    }),
  ).rejects.toMatchObject({ code: 'artifact-lease-mismatch' });
  await expect(inspectProviderEvaluationLock({ outputRoot: root, runId })).resolves.toMatchObject({
    lock: expect.objectContaining({ attemptId }),
  });
  ...
  await expect(Bun.file(retainedArtifact).text()).resolves.toBe('{"retained":true}\n');
});

test('reuses an evaluator-private terminal checkpoint only for the exact audit binding', async () => {
  const root = join(tmpdir(), `audit-evaluator-work-${crypto.randomUUID()}`);
...
test('round-trips every evaluator recovery leaf only after its exact topology completes', async () => {
  const root = join(tmpdir(), `audit-evaluator-leaves-${crypto.randomUUID()}`);
  await mkdir(root, { recursive: true });
  const draft = createDraftPlan({
    targetFingerprint: 'a'.repeat(64),
    contextDigest: 'b'.repeat(64),
