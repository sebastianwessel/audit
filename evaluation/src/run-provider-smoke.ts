...
import { createVerificationRouteFingerprint } from '../../src/features/review-workflow/runtime/verification-route.js';
import { createReviewService } from '../../src/features/review-workflow/service.js';
import { writeJsonArtifact } from '../../src/platform/artifact-store/json-artifact-store.js';
import {
...
export function parseProviderSmokeArguments(
  ...
  if (!parsed.success) throw usage('A provider and model are required for a provider smoke.');
  return parsed.data;
}
...
export async function runProviderSmoke(input: {
  ...
  const structuredOutputCompatibility = assertAuditWorkflowStructuredOutputCompatibility(
  ...
  try {
    ...
    const evaluatorFailureDiagnosticsStore = input.options.debugDiagnostics
      ? createEvaluatorFailureDiagnosticsStore({
          outputRoot: input.options.output,
          evaluationRunId: input.options.runId,
          privateWorkPath: `.provider-evaluations/${input.options.runId}/work`,
        })
      : undefined;
    const service = createReviewService(modelProvider, input.options.model, {
      ...
      ...(evaluatorFailureDiagnosticsStore === undefined
        ? {}
        : {
            evaluatorFailureDiagnosticSink: createEvaluatorFailureDiagnosticSink({
    ...
    const identity = ProviderSmokeIdentitySchema.parse({
      ...
      evidenceMapProtocolFingerprint,
      reviewWorkflowProtocolFingerprint: reviewWorkflowPromptProtocolFingerprint,
      structuredOutputCompatibilityFingerprint:
        structuredOutputCompatibility.compatibilityFingerprint,
...
export async function runProviderSmokeCommand(argv: readonly string[]): Promise<number> {
  if (isEvaluationHelpRequest(argv)) {
    process.stdout.write(`${providerSmokeUsage}\n`);
    return 0;
  }
...
function usage(message: string): AuditRuntimeError {
  return new AuditRuntimeError('invalid-input', `${message} ${providerSmokeUsage}`);
}

const providerSmokeUsage =
  'Usage: bun run eval:smoke --case <development-case-id> --variant <vulnerable|patched|benign> [--corpus evaluation/data/corpora --output evaluation/runs --run-id <id> --resume true --retry-unfinished true --debug-diagnostics true]';

if (import.meta.main) {
  try {
    process.exitCode = await runProviderSmokeCommand(Bun.argv.slice(2));
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Unexpected provider smoke failure.';
    process.stderr.write(`audit provider smoke: ${message}\n`);
    process.exitCode = 2;
  }
}
